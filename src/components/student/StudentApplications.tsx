...
import { getApplicationsForStudent, listOpenOpportunities, getProfilesByIds, type DBOpportunity } from "@/lib/db";
...
export function StudentApplications() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async (): Promise<Row[]> => {
      ...
      const opps = await Promise.all(apps.map(async (a) => {
        // We can't bulk-fetch via student RLS, but each opportunity is publicly readable.
        const { supabase } = await import("@/integrations/supabase/client");
        const { data } = await supabase.from("opportunities").select("*").eq("id", a.opportunity_id).maybeSingle();
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-10">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">Every program you've applied to, with live status.</p>
      </div>

      <div className="mt-6 sm:mt-8 overflow-hidden rounded-2xl border border-border bg-card">
        {error && (
          <div className="p-6 text-sm text-rose-500">Couldn't load applications. {(error as Error).message}</div>
        )}
        {showSkeleton && (
        ...
        {!showSkeleton && data && data.length === 0 && (
          <div className="p-10 sm:p-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Discover programs and apply to start tracking here.</p>
            <Link to="/dashboard"><Button className="mt-5">Discover opportunities</Button></Link>
          </div>
        )}
        {!showSkeleton && data && data.length > 0 && (
          <>
                ...
                <div key={r.id} className="p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1">
                    <span className="text-xs text-muted-foreground">
                      {new Date(r.applied_at).toLocaleDateString(undefined, { day: "numeric", month: "short", year: "numeric" })}
                    </span>
                    {r.status === "ACCEPTED" ? (
                      <Link to="/dashboard/room/$opportunityId" params={{ opportunityId: r.opportunity_id }}>
                        <Button size="sm" variant="outline">View Room</Button>
                      </Link>
                    ) : (
                ...
                <tbody>
                  {data.map((r) => (
                    <tr key={r.id} className="border-t border-border transition hover:bg-muted/30">
                      ...
                      <td className="px-5 py-4 text-muted-foreground">{new Date(r.applied_at).toLocaleDateString(undefined, { day: "numeric", month: "short", year: "numeric" })}</td>
                      <td className="px-5 py-4"><StatusPill status={r.status} /></td>
                      <td className="px-5 py-4 text-right">
                        {r.status === "ACCEPTED" ? (
                          <Link to="/dashboard/room/$opportunityId" params={{ opportunityId: r.opportunity_id }}>
                            <Button size="sm" variant="outline">View Notice Board</Button>
                          </Link>
                        ) : (
                          <span className="text-xs text-muted-foreground">—</span>
                        )}
